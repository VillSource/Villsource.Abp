...
public class AbpModule : AbpModule
{
    ...
    private void ConfigureAutoApiControllers()
    {
        ...
        {
            options.ConventionalControllers.Create(typeof(AbpModule).Assembly);
        });
    }
    ...
    {
        context.Services.AddDataProtection().SetApplicationName("Abp");
    }
    ...
    private void ConfigureVirtualFiles(IWebHostEnvironment hostingEnvironment)
    {
        ...
        {
            options.FileSets.AddEmbedded<AbpModule>();
            if (hostingEnvironment.IsDevelopment())
            {
                /* Using physical files in development, so we don't need to recompile on changes */
                options.FileSets.ReplaceEmbeddedByPhysical<AbpModule>(hostingEnvironment.ContentRootPath);
            }
        });
    }
